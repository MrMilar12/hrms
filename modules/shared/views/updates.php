    ...
    <div class="release-history-list">
        <?php foreach ($releases as $release): ?>
            <article class="glass-card release-history-item">
                ...
                <div><h2><?= htmlspecialchars($release['title']) ?></h2><div class="release-changes"><?= nl2br(htmlspecialchars($release['changes'])) ?></div><?php if (Auth::isDeveloper() && !empty($release['release_url'])): ?><a class="release-source-link" href="<?= htmlspecialchars($release['release_url']) ?>" target="_blank" rel="noopener noreferrer">View on GitHub &rarr;</a><?php endif; ?></div>
            </article>
        <?php endforeach; ?>
